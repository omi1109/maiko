...
import com.example.maico.Event.SignupEventPublisher;
import com.example.maico.entity.User;
import com.example.maico.entity.VerificationToken;
import com.example.maico.form.SignupForm;
import com.example.maico.service.UserService;
import com.example.maico.service.VerificationTokenService;
...
public class AuthController {
    ...
    public String signup(@ModelAttribute @Validated SignupForm signupForm, BindingResult bindingResult, RedirectAttributes redirectAttributes,HttpServletRequest httpServletRequest) {
    	...
    	if(userService.isEmailRegistered(signupForm.getEmail())) {
    		FieldError fieldError = new FieldError(bindingResult.getObjectName(), "email", "すでに登録済みのメールアドレスどす。");
    		
    	}
    	// パスワードとパスワード（確認用）の入力値が一致しなければ、BindingResultオブジェクトにエラー内容を追加する
    if (!userService.isSamePassword(signupForm.getPassword(),signupForm.getPasswordConfirmation())) {
    	FieldError fieldError = new FieldError(bindingResult.getObjectName(), "password", "パスワード一致しまへん。");
